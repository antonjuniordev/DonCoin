...
The RPM spec file provided here is for DonCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from http://doncoin.cc/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DonCoin git repository](https://github.com/doncoin-project/doncoin-core) but are not part of
...
Currently only `Source10` is used, for the example `doncoin.conf` file.
...
in the doncoin git repository. At present only an SVG version of the DonCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the doncoin
...
needed when 0.12.1 is released, a proper fix is already in the DonCoin git
master branch.
...
idea to build DonCoin Core against a static version of that library compiled
...
The `doncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `doncoin-server` package. At this point, most RPM based
...
The `doncoin-server` package should have SELinux support. How to properly do
...
DonCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/doncoin/). He has been
packaging DonCoin for Fedora at least since 2012.
...
4. I renamed the `doncoin` package that contains the Qt GUI to `doncoin-core` as
that appears to be how the general population refers to it, in contrast to
`doncoin-xt` or `doncoin-classic`. I wanted to make sure the general population
