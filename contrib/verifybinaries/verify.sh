...
###   This script attempts to download the signature file SHA256SUMS.asc from doncoin.org
...
WORKINGDIR="/tmp/doncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="http://doncoin.cc/bin/"
VERSIONPREFIX="doncoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'doncoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the DonCoin Core binary release signing key installed?"
   fi
