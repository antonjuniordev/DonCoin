...
#if defined(NDEBUG)
# error "DonCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "DonCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	int DNCFullblock = (42987 + fork1)*4;
	// 42987 blocks from BTC done with 12.5 BTC, 10000 blocks from prefork with 12.5 DNC
	if(nHeight <= fork1)
	    {
        return nSubsidy;
	    }
	    ...
	    int halvings = (nHeight+DNCFullblock) / nSubsidyHalvingInterval2;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // DonCoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
