DonCoin Core version 0.12.0 is now available from:

  <http://doncoin.cc/bin/doncoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/DonCoin-Qt (on Mac) or
doncoind/doncoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of DonCoin Core or other software:
...
pre-0.12 versions of DonCoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start DonCoin Core version 0.11 or
earlier.
...
DonCoin Core 0.12 will have a strict maximum size on the mempool. The
...
DonCoin Core 0.12 also introduces new default policy limits on the length and
...
DonCoin Core 0.12 nodes. DonCoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/doncoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in DonCoin Core 0.12 does not yet have support for
...
notes of v0.11.0](https://github.com/doncoin-project/doncoin-core/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
- #6214 `87406aa` [OSX] revert renaming of DonCoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new doncoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include doncoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix doncoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of doncoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override DONCOIND and DONCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [doncoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add DONCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libdoncoinconsensus (Braydon Fuller)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/doncoin/).
