...
*Setup instructions for a Gitian build of DonCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DonCoin
...
- [Building DonCoin Core](#building-doncoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-doncoin-host/README.md).
...
Clone the git repositories for doncoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/doncoin-project/doncoin
...
in the doncoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building DonCoin Core
----------------

To build DonCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the doncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/doncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/doncoin-project/doncoin
...
For example:
```bash
URL=https://github.com/thrasher-/doncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit doncoin=${COMMIT} --url doncoin=${URL} ../doncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit doncoin=${COMMIT} --url doncoin=${URL} ../doncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit doncoin=${COMMIT} --url doncoin=${URL} ../doncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the doncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../doncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/doncoin
SIGPATH=/some/root/path/doncoin-detached-sigs

./bin/gbuild --url doncoin=${BTCPATH},signature=${SIGPATH} ../doncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/doncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/doncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/doncoin-osx-build.assert
```
...
[doncoin-project/gitian.sigs.mac](https://github.com/doncoin-project/gitian.sigs.mac/) repository, or if that's not possible create a pull
